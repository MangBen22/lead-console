(function () {
  ...
  async function loadReleaseDecision() {
    ...
    try {
      ...
      const releaseDecisionHistoryDetailId = document.getElementById("releaseDecisionHistoryDetailId");
      if (releaseDecisionHistoryDetailId && data && data.history && data.history.snapshot_id) {
        releaseDecisionHistoryDetailId.value = String(data.history.snapshot_id);
      }
    } catch (err) {
  ...
  async function downloadReleaseDecision() {
    const releaseDecisionView = document.getElementById("releaseDecisionView");
    const data = await apiGetWithParams("deployment.release.decision.export", {
      freshness_minutes: releaseGateFreshnessInput && releaseGateFreshnessInput.value ? Number(releaseGateFreshnessInput.value) : 30,
    });
    const filename = data && data.filename ? String(data.filename) : ("release-decision-" + Date.now() + ".json");
    downloadJsonFile(filename, data.export || data);
    if (releaseDecisionView) {
      releaseDecisionView.textContent = JSON.stringify(data, null, 2);
    }
    const releaseDecisionHistoryDetailId = document.getElementById("releaseDecisionHistoryDetailId");
    if (releaseDecisionHistoryDetailId && data && data.history && data.history.snapshot_id) {
  ...
  async function loadReleaseDecisionHistory() {
    const releaseDecisionHistoryView = document.getElementById("releaseDecisionHistoryView");
    if (!releaseDecisionHistoryView) return;
    try {
      const data = await apiGetWithParams("deployment.release.decision.history", {
      ...
      releaseDecisionHistoryView.textContent = "Failed to load release decision history.";
    }
  }
  ...
  async function loadReleaseDecisionHistorySummary() {
    ...
    try {
      const data = await apiGetWithParams("deployment.release.decision.history_summary", {
        limit: document.getElementById("releaseDecisionHistoryLimit") && document.getElementById("releaseDecisionHistoryLimit").value ? Number(document.getElementById("releaseDecisionHistoryLimit").value) : 20,
      });
      releaseDecisionHistorySummaryView.textContent = JSON.stringify(data, null, 2);
    } catch (err) {
      releaseDecisionHistorySummaryView.textContent = "Failed to load release decision history summary.";
    }
  }

  async function downloadReleaseDecisionHistorySummary() {
    const releaseDecisionHistorySummaryView = document.getElementById("releaseDecisionHistorySummaryView");
    const data = await apiGetWithParams("deployment.release.decision.history_summary_export", {
      limit: document.getElementById("releaseDecisionHistoryLimit") && document.getElementById("releaseDecisionHistoryLimit").value ? Number(document.getElementById("releaseDecisionHistoryLimit").value) : 20,
    });
    const filename = data && data.filename ? String(data.filename) : ("release-decision-history-summary-" + Date.now() + ".json");
    downloadJsonFile(filename, data.export || data);
    if (releaseDecisionHistorySummaryView) {
      releaseDecisionHistorySummaryView.textContent = JSON.stringify(data, null, 2);
    }
  }
  ...
  async function loadReleaseDecisionIssuesSummary() {
    const releaseDecisionIssuesSummaryView = document.getElementById("releaseDecisionIssuesSummaryView");
    if (!releaseDecisionIssuesSummaryView) return;
    try {
      const data = await apiGetWithParams("deployment.release.decision.issues_summary", {
        freshness_minutes: releaseGateFreshnessInput && releaseGateFreshnessInput.value ? Number(releaseGateFreshnessInput.value) : 30,
      });
      releaseDecisionIssuesSummaryView.textContent = JSON.stringify(data, null, 2);
    } catch (err) {
      releaseDecisionIssuesSummaryView.textContent = "Failed to load release decision issues summary.";
    }
  }

  async function loadReleaseDecisionActionPlan() {
    const releaseDecisionActionPlanView = document.getElementById("releaseDecisionActionPlanView");
    if (!releaseDecisionActionPlanView) return;
    try {
      const data = await apiGetWithParams("deployment.release.decision.action_plan", {
        freshness_minutes: releaseGateFreshnessInput && releaseGateFreshnessInput.value ? Number(releaseGateFreshnessInput.value) : 30,
      });
      releaseDecisionActionPlanView.textContent = JSON.stringify(data, null, 2);
    } catch (err) {
      releaseDecisionActionPlanView.textContent = "Failed to load release decision action plan.";
    }
  }
  ...
  async function downloadReleaseDecisionActionPlan() {
    ...
    const filename = data && data.filename ? String(data.filename) : ("release-decision-action-plan-" + Date.now() + ".json");
    downloadJsonFile(filename, data.export || data);
    if (releaseDecisionActionPlanView) {
      releaseDecisionActionPlanView.textContent = JSON.stringify(data, null, 2);
    }
  }
  ...
  async function downloadReleaseDecisionReviewBundle() {
    const releaseDecisionView = document.getElementById("releaseDecisionView");
    const decisionLimit = document.getElementById("releaseDecisionHistoryLimit");
    const data = await apiGetWithParams("deployment.release.decision.review_bundle", {
      limit: decisionLimit && decisionLimit.value ? Number(decisionLimit.value) : 20,
      freshness_minutes: releaseGateFreshnessInput && releaseGateFreshnessInput.value ? Number(releaseGateFreshnessInput.value) : 30,
    });
    const filename = data && data.filename ? String(data.filename) : ("release-decision-review-bundle-" + Date.now() + ".json");
    downloadJsonFile(filename, data.export || data);
    if (releaseDecisionView) {
      releaseDecisionView.textContent = JSON.stringify(data, null, 2);
    }
  ...
  async function loadReleaseDecisionHistoryDetail() {
    ...
    try {
      const data = await apiGetWithParams("deployment.release.decision.history.detail", {
        snapshot_id: id,
      });
      releaseDecisionHistoryDetailView.textContent = JSON.stringify(data, null, 2);
    } catch (err) {
      releaseDecisionHistoryDetailView.textContent = "Failed to load release decision history detail.";
    }
  }
  ...
  const downloadReleaseDecisionBtn = document.getElementById("downloadReleaseDecisionBtn");
  if (downloadReleaseDecisionBtn) {
    downloadReleaseDecisionBtn.addEventListener("click", async function () {
  ...
  const refreshReleaseDecisionHistoryBtn = document.getElementById("refreshReleaseDecisionHistoryBtn");
  if (refreshReleaseDecisionHistoryBtn) {
  ...
  const downloadReleaseDecisionHistorySummaryBtn = document.getElementById("downloadReleaseDecisionHistorySummaryBtn");
  if (downloadReleaseDecisionHistorySummaryBtn) {
    downloadReleaseDecisionHistorySummaryBtn.addEventListener("click", async function () {
      await downloadReleaseDecisionHistorySummary();
    });
  }
  ...
  const downloadReleaseDecisionActionPlanBtn = document.getElementById("downloadReleaseDecisionActionPlanBtn");
  ...
  const downloadReleaseDecisionReviewBundleBtn = document.getElementById("downloadReleaseDecisionReviewBundleBtn");
  if (downloadReleaseDecisionReviewBundleBtn) {
    downloadReleaseDecisionReviewBundleBtn.addEventListener("click", async function () {
      await downloadReleaseDecisionReviewBundle();
    });
  }

  const loadReleaseDecisionHistoryDetailBtn = document.getElementById("loadReleaseDecisionHistoryDetailBtn");
