...
function crm_email_template_test_log_snapshot($query = [])
{
    ...
    $siteId = strtolower(trim((string) ($query['site_id'] ?? '')));
    $success = strtolower(trim((string) ($query['success'] ?? '')));
    $templateKey = strtolower(trim((string) ($query['template_key'] ?? '')));
    $errorCode = strtolower(trim((string) ($query['error_code'] ?? '')));
    $search = strtolower(trim((string) ($query['search'] ?? '')));
    ...
    foreach ($sites as $site) {
        ...
        $summary['site_count']++;
        $snapshot = crm_email_template_test_log_fetch($site, 50);
        ...
        foreach ($rows as $row) {
            ...
            $item = $row;
            $item['site_id'] = (string) ($site['site_id'] ?? '');
            $item['site_label'] = (string) ($site['label'] ?? $site['base_url']);
            $itemSuccess = !empty($item['success']);
            $summary['total_count']++;
            if ($itemSuccess) {
            ...
            if ($success !== '') {
                $successKey = $itemSuccess ? 'success' : 'failed';
                if ($successKey !== $success) {
                    continue;
                }
            }
            if ($templateKey !== '' && strpos(strtolower((string) ($item['template_key'] ?? '')), $templateKey) === false) {
                continue;
            }
            if ($errorCode !== '' && strpos(strtolower((string) ($item['error_code'] ?? '')), $errorCode) === false) {
                continue;
            }
            if ($search !== '') {
                $matched = false;
    ...
    $summary['filtered_count'] = count($items);
    $offset = ($page - 1) * $limit;
...
function crm_operations_history_path()
{
    return app_storage_path('crm_operations_history.json');
}

function crm_operations_record_snapshot($snapshot, $source = 'manual_view')
{
    $payload = is_array($snapshot) ? $snapshot : [];
    $entry = [
        'snapshot_id' => 'crm_ops_' . gmdate('Ymd_His') . '_' . substr(sha1((string) mt_rand()), 0, 6),
        'created_at' => gmdate('c'),
        'source' => (string) $source,
        'snapshot' => $payload,
        'summary' => isset($payload['summary']) && is_array($payload['summary']) ? $payload['summary'] : [],
    ];
    $rows = app_read_json_file(crm_operations_history_path(), []);
    array_unshift($rows, $entry);
    $rows = array_slice(array_values(array_filter($rows, static function ($row) {
        return is_array($row);
    })), 0, 200);
    app_write_json_file(crm_operations_history_path(), $rows);
    return $entry;
}

function crm_operations_history_snapshot($limit = 10)
{
...
function crm_operations_latest_compare()
{
    $history = crm_operations_history_snapshot(2);
    $items = isset($history['items']) && is_array($history['items']) ? $history['items'] : [];
    ...
    $keys = [
        'connector_count',
        'active_connectors',
        'delivery_connectors_with_retries',
        'delivery_connectors_with_rejections',
        'retry_filtered_count',
        'smtp_connected_sites',
        'smtp_failed_sites',
        'template_site_count',
        'template_log_filtered_count',
    ];
    $changes = [];
    foreach ($keys as $key) {
        $changes[$key] = [
...
function webops_operations_history_path()
{
    return app_storage_path('webops_operations_history.json');
}

function webops_operations_record_snapshot($snapshot, $source = 'manual_view')
{
    $payload = is_array($snapshot) ? $snapshot : [];
    $entry = [
        'snapshot_id' => 'webops_ops_' . gmdate('Ymd_His') . '_' . substr(sha1((string) mt_rand()), 0, 6),
        'created_at' => gmdate('c'),
        'source' => (string) $source,
        'snapshot' => $payload,
        'summary' => isset($payload['summary']) && is_array($payload['summary']) ? $payload['summary'] : [],
    ];
    $rows = app_read_json_file(webops_operations_history_path(), []);
    array_unshift($rows, $entry);
    $rows = array_slice(array_values(array_filter($rows, static function ($row) {
        return is_array($row);
    })), 0, 200);
    app_write_json_file(webops_operations_history_path(), $rows);
    return $entry;
}

function webops_operations_history_snapshot($limit = 10)
{
    $safeLimit = max(1, min(100, (int) $limit));
    $rows = app_read_json_file(webops_operations_history_path(), []);
    $rows = array_values(array_filter($rows, static function ($row) {
...
function seo_operations_history_summary($limit = 20)
{
    ...
    $totals = [
        ...
        'latest_audit_score' => 0,
        'latest_critical_issues' => 0,
        'extension_event_count' => 0,
        'active_extension_sessions' => 0,
        'regression_projects' => 0,
        'improved_projects' => 0,
    ];
    foreach ($items as $item) {
        $summary = isset($item['summary']) && is_array($item['summary']) ? $item['summary'] : [];
        foreach (array_keys($totals) as $key) {
            $totals[$key] += (float) ($summary[$key] ?? 0);
        }
    }
    $averages = [];
    foreach ($totals as $key => $total) {
    ...
    $changes = [];
    foreach (array_keys($totals) as $key) {
        $changes[$key] = round(((float) ($latestSummary[$key] ?? 0)) - ((float) ($oldestSummary[$key] ?? 0)), 2);
    }
...
function deployment_release_log_latest_compare()
{
    $rows = app_read_json_file(deployment_release_log_path(), []);
    $rows = array_values(array_filter($rows, static function ($row) {
        return is_array($row);
    }));
    $latest = isset($rows[0]) && is_array($rows[0]) ? $rows[0] : null;
    $previous = isset($rows[1]) && is_array($rows[1]) ? $rows[1] : null;
    $keys = [
        'release_gate_allowed',
        'guard_allowed',
        'launch_issue_count',
    ...
    foreach ($keys as $key) {
        $changes[$key] = [
            'latest' => (int) (($latest[$key] ?? 0)),
            'previous' => (int) (($previous[$key] ?? 0)),
            'delta' => (int) (($latest[$key] ?? 0)) - (int) (($previous[$key] ?? 0)),
        ];
    }
    return [
        'summary' => [
            'has_latest' => is_array($latest) ? 1 : 0,
            'has_previous' => is_array($previous) ? 1 : 0,
            'latest_candidate_id' => (string) ($latest['candidate_id'] ?? ''),
            ...
            'latest_launch_state' => (string) ($latest['launch_state'] ?? ''),
            'previous_launch_state' => (string) ($previous['launch_state'] ?? ''),
        ],
        'latest' => $latest,
        'previous' => $previous,
        'changes' => $changes,
...
function seo_extension_events_list_snapshot($query = [])
{
    ...
    foreach ($rows as $row) {
        ...
        $itemSessionId = (string) ($item['session_id'] ?? '');
        $sessionKey = $itemSessionId !== '' ? $itemSessionId : 'unassigned';
        $projectKey = $itemProjectId !== '' ? $itemProjectId : 'unassigned';
        if (!isset($sessionCounts[$sessionKey])) {
            $sessionCounts[$sessionKey] = 0;
        }
...
if ($action === 'deployment.release.log.latest_compare') {
    $compare = deployment_release_log_latest_compare();
    out_json([
        'ok' => true,
...
if ($action === 'deployment.release.log.latest_compare_export') {
    $compare = deployment_release_log_latest_compare();
    audit_event('deployment', 'release.log.latest_compare.export', [
        'has_latest' => (int) ($compare['summary']['has_latest'] ?? 0),
        'has_previous' => (int) ($compare['summary']['has_previous'] ?? 0),
        'latest_candidate_id' => (string) ($compare['summary']['latest_candidate_id'] ?? ''),
    ]);
    out_json([
        'ok' => true,
        'filename' => 'release_log_latest_compare_' . gmdate('Ymd_His') . '.json',
        'export' => $compare,
        'time' => gmdate('c'),
    ]);
}
...
if ($action === 'deployment.release.decision.latest_compare_export') {
    $compare = deployment_release_decision_latest_compare();
    audit_event('deployment', 'release.decision.latest_compare.export', [
        'has_latest' => (int) ($compare['summary']['has_latest'] ?? 0),
        'has_previous' => (int) ($compare['summary']['has_previous'] ?? 0),
        'latest_snapshot_id' => (string) ($compare['summary']['latest_snapshot_id'] ?? ''),
        'latest_decision' => (string) ($compare['summary']['latest_decision'] ?? ''),
    ]);
    out_json([
        'ok' => true,
...
if ($action === 'crm.email_templates.test_log') {
    $snapshot = crm_email_template_test_log_snapshot($_GET);
    out_json([
        'ok' => true,
        'summary' => $snapshot['summary'],
        'items' => $snapshot['items'],
...
if ($action === 'crm.email_templates.test_log_export') {
    ...
    $payload = [
        'exported_at' => gmdate('c'),
        'test_log' => crm_email_template_test_log_snapshot($filters),
    ];
    if ($logId !== '') {
        $payload['log_detail'] = crm_email_template_test_log_detail_snapshot($logId, $detailSiteId !== '' ? $detailSiteId : ($filters['site_id'] ?? ''));
    }
    audit_event('crm', 'email_templates.test_log.export', [
        'log_id' => $logId,
        'detail_site_id' => $detailSiteId,
        'limit' => $limit,
        'filtered_count' => (int) (($payload['test_log']['summary']['filtered_count'] ?? 0)),
    ]);
    out_json([
        'ok' => true,
...
if ($action === 'crm.operations.snapshot') {
    ...
    $snapshot = crm_operations_snapshot($limit);
    $record = crm_operations_record_snapshot($snapshot, 'manual_view');
    out_json([
        'ok' => true,
        'snapshot' => $snapshot,
        'record' => $record,
...
if ($action === 'crm.operations.export') {
    $limit = isset($_GET['limit']) ? (int) $_GET['limit'] : 10;
    if ($limit <= 0) {
        $limit = 10;
    }
    $snapshot = crm_operations_snapshot($limit);
    audit_event('crm', 'operations.export', [
        'limit' => $limit,
        'connector_count' => (int) (($snapshot['summary']['connector_count'] ?? 0)),
    ]);
    out_json([
        'ok' => true,
        'filename' => 'crm_operations_snapshot_' . gmdate('Ymd_His') . '.json',
        'export' => $snapshot,
    ]);
}

if ($action === 'crm.operations.history') {
    $limit = isset($_GET['limit']) ? (int) $_GET['limit'] : 10;
    if ($limit <= 0) {
        $limit = 10;
    }
...
if ($action === 'crm.operations.latest_compare') {
    $compare = crm_operations_latest_compare();
    out_json([
        'ok' => true,
        'summary' => $compare['summary'],
        'latest' => $compare['latest'],
...
if ($action === 'webops.operations.snapshot') {
    ...
    $snapshot = webops_operations_snapshot($limit);
    $record = webops_operations_record_snapshot($snapshot, 'manual_view');
    out_json([
        'ok' => true,
        'snapshot' => $snapshot,
        'record' => $record,
...
if ($action === 'webops.operations.history') {
    $limit = isset($_GET['limit']) ? (int) $_GET['limit'] : 10;
    if ($limit <= 0) {
        $limit = 10;
    }
...
if ($action === 'seo.operations.export') {
    $limit = isset($_GET['limit']) ? (int) $_GET['limit'] : 10;
    if ($limit <= 0) {
        $limit = 10;
    }
    $snapshot = seo_operations_snapshot($limit);
    audit_event('seo', 'operations.export', [
        'limit' => $limit,
        'project_count' => (int) (($snapshot['summary']['project_count'] ?? 0)),
    ]);
    out_json([
        'ok' => true,
...
if ($action === 'seo.operations.history_summary') {
    $limit = isset($_GET['limit']) ? (int) $_GET['limit'] : 20;
    if ($limit <= 0) {
        $limit = 20;
    }
    $summary = seo_operations_history_summary($limit);
    out_json([
        'ok' => true,
        'summary' => $summary['summary'],
        'latest' => $summary['latest'],
...
if ($action === 'launch.operations.source_summary_export') {
    $summary = launch_operations_source_summary_snapshot([
        'source' => isset($_GET['source']) ? (string) $_GET['source'] : '',
        'launch_state' => isset($_GET['launch_state']) ? (string) $_GET['launch_state'] : '',
        'search' => isset($_GET['search']) ? (string) $_GET['search'] : '',
    ]);
    audit_event('launch', 'operations.source_summary.export', [
        'filtered_count' => (int) ($summary['summary']['filtered_count'] ?? 0),
        'source_count' => (int) ($summary['summary']['source_count'] ?? 0),
    ]);
    out_json([
        'ok' => true,
        'filename' => 'launch_operations_source_summary_' . gmdate('Ymd_His') . '.json',
        'export' => $summary,
    ]);
}
...
if ($action === 'launch.operations.history_summary_export') {
    $limit = isset($_GET['limit']) ? (int) $_GET['limit'] : 20;
    if ($limit <= 0) {
        $limit = 20;
    }
    $summary = launch_operations_history_summary($limit);
    audit_event('launch', 'operations.history_summary.export', [
        'limit' => $limit,
        'runs_count' => (int) ($summary['summary']['runs_count'] ?? 0),
        'latest_launch_state' => (string) ($summary['summary']['latest_launch_state'] ?? ''),
    ]);
    out_json([
        'ok' => true,
        'filename' => 'launch_operations_history_summary_' . gmdate('Ymd_His') . '.json',
        'export' => $summary,
    ]);
}
...
if ($action === 'seo.extension.events.summary.export') {
    $events = app_read_json_file(seo_extension_events_path(), []);
    $projectId = isset($_GET['project_id']) ? (string) $_GET['project_id'] : '';
    if ($projectId !== '') {
        $events = array_values(array_filter($events, static function ($row) use ($projectId) {
    ...
    audit_event('seo', 'extension.events.summary.export', [
        'project_id' => $projectId,
        'event_count' => count($events),
    ]);
    out_json([
        'ok' => true,
        'filename' => 'seo_extension_summary_export_' . gmdate('Ymd_His') . '.json',
        'export' => [
            'exported_at' => gmdate('c'),
            'event_count' => count($events),
            'average_score' => count($events) > 0 ? round($scoreTotal / count($events), 2) : 0,
...
if ($action === 'seo.opportunities.export') {
    $projectId = isset($_GET['project_id']) ? (string) $_GET['project_id'] : '';
    $projects = app_read_json_file(seo_projects_path(), []);
    $project = null;
    if ($projectId !== '') {
    ...
    if (is_array($project)) {
        ...
        $decliningUrls = array_values(array_filter($historySnapshot['items'], static function ($item) {
            return isset($item['score_delta']) && $item['score_delta'] !== null && (int) ($item['score_delta'] ?? 0) < 0;
        }));
        usort($decliningUrls, static function ($left, $right) {
            return (int) ($left['score_delta'] ?? 0) <=> (int) ($right['score_delta'] ?? 0);
        });
    ...
    audit_event('seo', 'opportunities.export', [
        'project_id' => $projectId,
        'recurring_issue_count' => (int) ($payload['summary']['recurring_issue_count'] ?? 0),
    ]);
    out_json([
        'ok' => true,
        'filename' => 'seo_opportunities_export_' . gmdate('Ymd_His') . '.json',
        'export' => $payload,
    ]);
}
