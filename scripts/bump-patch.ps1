...
if ($versionMatch.Groups[4].Success) {
    ...
    $oldVersion = "$major.$minor.$patch.$($build - 1)"
} else {
    $patch++
...
$content = [regex]::Replace($content, "Version:\s*[0-9]+\.[0-9]+\.[0-9]+(?:\.[0-9]+)?", "Version: $newVersion", 1)
$content = [regex]::Replace($content, "define\('LC_PLUGIN_VERSION',\s*'[0-9]+\.[0-9]+\.[0-9]+(?:\.[0-9]+)?'\);", "define('LC_PLUGIN_VERSION', '$newVersion');", 1)
