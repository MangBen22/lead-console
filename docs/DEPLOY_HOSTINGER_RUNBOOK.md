...
10. In `Go-Live Status`, configure release gate settings (`freshness window` + required checks, including optional `Require recent cutover signoff`), click `Save Release Gate Settings`, then `Refresh Release Gate`.
